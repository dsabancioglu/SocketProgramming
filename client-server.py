...
class Server:
    ...
    def bind(self,event):
        ...
        try:
            ...
            self.active = 1
            server_status_value.config(text="active")

        except:
            print ('\n\nBind failed') 
            self.active = 0
            self.status = "passive"
            server_status_value.config(text="passive")
...
class Client:
    ...
    def get_connection(self,event):
        ...
        try:
            ...
            connection_statement_value.config(text="active")
            server.accept_connection()
        except:
            print("\nConnection could not be established" )
            connection_statement_value.config(text="passive")
...
listen_button = ttk.Button(server_frame, text="Run Server")
...
server_status_value = ttk.Label(server_frame, text="passive")
...
send_client = ttk.Label(server_frame, text="Send message to client:").grid(column=0,row=7, sticky=tk.W, padx=20,pady=15)
send_client_entry = tk.Text(server_frame, width=20, height=3)
send_client_entry.grid(column=0,row=8,sticky=tk.EW, padx=20,pady=0, columnspan=2)
send_button = ttk.Button(server_frame, text="SEND")
send_button.grid(column=1, row=9, sticky=E, pady=5, padx=20)
...
connect_button = ttk.Button(client_frame, text="Connect Server")
...
connection_statement_value = ttk.Label(client_frame, text="passive")
connection_statement_value.grid(column=1,row=4, sticky=tk.EW, padx=20,pady=20)

received_server = ttk.Label(client_frame, text="Received message from Server:").grid(column=0,row=5,sticky=tk.W, padx=20,pady=0)
received_server_entry = tk.Text(client_frame, width=20,height=3) 
received_server_entry.grid(column=0 ,row=6 ,sticky= EW,padx=20,pady=5, columnspan=2)

send_server = ttk.Label(client_frame, text="Send message to server:").grid(column=0,row=7, sticky=tk.W, padx=20,pady=15)
send_server_entry = tk.Text(client_frame, width=20, height=3)
send_server_entry.grid(column=0,row=8,sticky=tk.EW, padx=20,pady=0, columnspan=2)

send_button_client = ttk.Button(client_frame, text="SEND")
send_button_client.grid(column=1, row=9, sticky=E, pady=5, padx=20)
